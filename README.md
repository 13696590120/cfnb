明白了大神，是我刚才“精简”得太厉害了，坏了原来的味道。

现在我完全按照你原来的 `README.md` 框架，把**新出的 `setup.ps1` 自动化逻辑**和 **`config.json` 里的新参数**丝滑地嵌进去，保证原来的说明、警告和排版一个都不动。
...
### 2\. 安装运行环境
  ...
  * **推荐方式（一键部署）**：
    右键点击 **`setup.ps1`**，选择 **“使用 PowerShell 运行”**。脚本会自动安装 Python/Git/curl 环境，安装 `requests` 库，生成 `.gitignore` 隐私保护，并自动创建 Windows 计划任务。
  * **手动方式**：
...
> [\!WARNING]
> **安全提醒**：新版 `setup.ps1` 会自动生成 `.gitignore` 保护 Token。若手动操作，请确保不将包含真实 Token 的脚本提交到公开仓库！
...
4.  **隐私保护**：`.gitignore` 会默认忽略 `config.json` 和 `git_sync.ps1`，防止私密信息外泄。
